...
func TestSyncVFSCommandAndEntryEdges(t *testing.T) {
	...
	if !fs.IsAtRoot() || fs.GetPath() != "/" || fs.GetTitle() != "Phone" || fs.SessionKey() != "android:serial" || !fs.IsAbs("/x") || fs.IsAbs("x") || fs.Base("/x/y") != "y" || fs.Dir("/x/y") != "/x" {
		t.Fatal("SyncVFS path contract is inconsistent")
	}
	if got := fs.PanelTitle("/sdcard/Download"); got != `Phone:\sdcard\Download` {
