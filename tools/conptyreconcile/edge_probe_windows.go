...
type edgeProbeReport struct {
	...
	SpacesEightTopTrimmed     bool               `json:"spaces_eight_top_trimmed"`
	SpacesNineTopTrimmed      bool               `json:"spaces_nine_top_trimmed"`
...
func runNativeEdgeProbe(hostPath, reportPath string) error {
	...
	report := edgeProbeReport{
		...
		Host:                      identity,
		Session:                   session,
		SpacesEightTopTrimmed:     find([]byte("spaces-eight-top:")) && bytes.Contains(session.RawOutput, []byte("spaces-eight-top:\r\n")),
		SpacesNineTopTrimmed:      find([]byte("spaces-nine-top:")) && bytes.Contains(session.RawOutput, []byte("spaces-nine-top:\r\n")),
	...
	if !report.SpacesEightTopTrimmed || !report.SpacesNineTopTrimmed || !report.SpacesEightBottomAdvanced || !report.SpacesNineBottomAdvanced || !report.BlinkRendered || report.BlinkInRenderedHistory || report.AutoWrapHostSequences != 0 || !report.AutoWrapTailLost {
