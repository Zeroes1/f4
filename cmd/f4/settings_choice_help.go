...
//go:embed settings_choice_help.tsv
	...
	if vtui.FrameManager != nil {
		for _, row := range c.page.rows {
			...
			if index >= 0 && index < len(row.field.Choices) {
				...
				text := row.field.Label.Resolve(AppConfig.Language, Msg) + " — " + choice.Label.Resolve(AppConfig.Language, Msg) + "\n\n" + description.Resolve(AppConfig.Language, Msg)
				if row.field.Timing != "" {
					text += "\n\n" + fmt.Sprintf(settingsPhrase("Takes effect: %s"), settingsPhrase(row.field.Timing))
				}
				if text != c.help.text {
					c.help.text = text
					c.help.top = 0
