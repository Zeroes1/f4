...
func newSettingsChord(value string, change func(string)) *settingsChord {
	...
	c.capture.OnClick = func() {
		hm := &HotkeyManager{Bindings: map[string]map[string]string{}, Defaults: map[string]map[string]string{}}
		frame := NewHotkeyAssignFrame(hm, "Settings.Capture", "Common", func() {
