...
func BenchmarkSettingsHoverSearch(b *testing.B) {
	for _, query := range []string{"", "editor"} {
		...
		b.Run(name, func(b *testing.B) {
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				c.ProcessMouse(&vtinput.InputEvent{Type: vtinput.MouseEventType, MouseX: int16(c.page.X1 + 5), MouseY: int16(c.page.Y1 + 1 + i%8), MouseEventFlags: vtinput.MouseMoved})
				c.Show(scr)
			}
		})
	}
