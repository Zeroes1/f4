...
func TestSettingsDropdownHoverHelpDoesNotChangeDraft(t *testing.T) {
	...
	for iteration, language := range []string{"en", "ru"} {
		...
		combo.Menu.ProcessMouse(&vtinput.InputEvent{Type: vtinput.MouseEventType, MouseX: int16(combo.Menu.X1 + 1), MouseY: int16(combo.Menu.Y1 + 3), MouseEventFlags: vtinput.MouseMoved})
		c.Show(scr)
