...
func newCoreRecordSettingsProvider() coreRecordSettingsProvider {
	...
	p.stores = append(p.stores, settingsRecordStore{collection: assoc, path: path, load: func() ([]f4settings.Record, error) {
		...
		var items []FileAssoc
		for _, r := range rows {
			item := FileAssoc{Mask: r.Values["assoc.Mask"], Description: r.Values["assoc.Description"]}
			for i, name := range assocKeyName {
		...
		return SaveAssociations(path, items)
	}})
	bookmarkPath := BookmarksFilePath()
	...
	p.stores = append(p.stores, settingsRecordStore{collection: bookmarks, path: bookmarkPath, load: func() ([]f4settings.Record, error) {
		items, err := LoadBookmarks(bookmarkPath)
		var rows []f4settings.Record
		for i, item := range items {
		...
		for i, r := range rows {
			items[i] = Bookmark{Path: r.Values["bookmark.Path"], Plugin: r.Values["Plugin"], PluginData: r.Values["PluginData"], PluginFile: r.Values["PluginFile"]}
		}
		return SaveBookmarks(bookmarkPath, items)
	}})
	linkPath := DriveBookmarksFilePath()
	...
	p.stores = append(p.stores, settingsRecordStore{collection: links, path: linkPath, load: func() ([]f4settings.Record, error) {
		items, err := LoadDriveBookmarks(linkPath)
		var rows []f4settings.Record
		for i, item := range items {
		...
		var items []DriveBookmark
		for _, r := range rows {
			items = append(items, DriveBookmark{Name: r.Values["link.Name"], Path: r.Values["link.Path"], Hotkey: r.Values["link.Hotkey"]})
		}
		return SaveDriveBookmarks(linkPath, items)
	}})
	p.addUserMenuStores()
	return p
}
...
func (p *coreRecordSettingsProvider) addUserMenuStores() {
	sources := []settingsMenuSource{{"main", "Global user menu", MainMenuFilePath(), MenuModeMain}}
	if exe, err := os.Executable(); err == nil {
		sources = append(sources, settingsMenuSource{"binary", "Executable user menu", filepath.Join(filepath.Dir(exe), farMenuFileName), MenuModeFar})
	}
	dir, _ := os.Getwd()
	if pf := findPanelsFrameAnyScreen(); pf != nil {
		if fsp, ok := pf.panels[pf.activeIdx].(*FileSystemPanel); ok && fsp.vfs != nil {
			if st, err := os.Stat(fsp.vfs.GetPath()); err == nil && st.IsDir() {
				dir = fsp.vfs.GetPath()
			}
		}
	}
	local, found := findLocalFarMenu(dir)
	if !found {
		local = filepath.Join(dir, farMenuFileName)
	}
	sources = append(sources, settingsMenuSource{"local", "Local/ancestor user menu", local, MenuModeLocal})
...
func newUserMenuSettingsStore(src settingsMenuSource) settingsRecordStore {
	...
	return settingsRecordStore{collection: col, path: src.path, load: func() ([]f4settings.Record, error) {
		...
		var err error
		if src.mode == MenuModeMain {
			items, err = LoadMainMenu(src.path)
		} else {
			items, err = loadFarMenuFile(src.path)
			if os.IsNotExist(err) {
				err = nil
			}
		}
		var rows []f4settings.Record
		var walk func([]UserMenuItem, string)
		walk = func(items []UserMenuItem, parent string) {
		...
		return saveRootForMode(src.mode, src.path, items)
	}}
}

func settingsMenuTree(rows []f4settings.Record, prefix string) ([]UserMenuItem, error) {
	...
	var build func(string) []UserMenuItem
	build = func(parent string) []UserMenuItem {
		items := []UserMenuItem{}
		for _, r := range rows {
			if r.Values[prefix+"Parent"] != parent {
				continue
			}
			item := UserMenuItem{Label: r.Values[prefix+"Label"], HotKey: r.Values[prefix+"HotKey"]}
...
func (p coreRecordSettingsProvider) Catalog() f4settings.Catalog {
	c := f4settings.Catalog{ID: "core-records", Categories: settingsCategories, Background: true}
...
func (p coreRecordSettingsProvider) Begin(context.Context) (*f4settings.Draft, error) {
	...
	d.CommitFunc = func(ctx context.Context, d *f4settings.Draft) f4settings.Result {
		...
		for _, s := range p.stores {
			...
			if s.afterSave != nil {
				settingsRunOnUI(ctx, func() { s.afterSave(rows) })
			}
