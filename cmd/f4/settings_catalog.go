...
var settingsCategories = []f4settings.Category{
...
func coreSettingsFields() []f4settings.Field {
	...
	for _, line := range strings.Split(strings.TrimSpace(coreSettingsDefinitions), "\n") {
		...
		v := settingsConfigField(reflect.ValueOf(AppConfig), f.ID)
		switch v.Kind() {
		case reflect.Bool:
...
func init() {
	for i := range settingsCategories {
		settingsCategories[i].Label.Key = "SettingsCenter.Category." + settingsCategories[i].ID
	}
}
