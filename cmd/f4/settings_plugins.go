...
func (pluginSettingsProvider) Catalog() f4settings.Catalog {
	...
	return f4settings.Catalog{ID: "plugins", Categories: settingsCategories, Collections: []f4settings.Collection{paths, grants}}
}
func (pluginSettingsProvider) Begin(context.Context) (*f4settings.Draft, error) {
	var paths, grants []f4settings.Record
	for i, path := range AppConfig.RegisteredPlugins {
		paths = append(paths, f4settings.Record{ID: fmt.Sprintf("plugin:%d", i), Values: map[string]string{"plugin.Path": path}})
	}
	for _, g := range PluginPermissions().Grants() {
		grants = append(grants, f4settings.Record{ID: "grant:" + g.Plugin + ":" + g.Permission, Values: map[string]string{"grant.Name": PermissionGrantLine(g), "grant.Plugin": g.Plugin, "grant.Permission": g.Permission, "grant.Decision": g.Decision, "grant.Keep": "true"}})
	}
	...
	d.CommitFunc = func(ctx context.Context, d *f4settings.Draft) f4settings.Result {
		...
		if d.Dirty("plugins.registered") {
			...
			if err := writeSettingsCandidate(before, next); err != nil {
				...
				AppConfig.RegisteredPlugins = next.RegisteredPlugins
