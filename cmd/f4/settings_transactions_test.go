...
func TestSettingsFrontendRegistrationAndRemoval(t *testing.T) {
	...
	reg, err := (&coreAPI{}).RegisterSettingsProvider(p)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSettingsApplyThenEditCancelKeepsAppliedBaseline(t *testing.T) {
	...
	defer func() { AppConfig = before; writeSettingsCandidate = writer }()
	writes := 0
	writeSettingsCandidate = func(F4Config, F4Config) error { writes++; return nil }
	...
	d.Close()
	if !AppConfig.ShowHiddenFiles || writes != 1 {
...
func TestSettingsSearchCollectionsAndCrossCategoryNavigation(t *testing.T) {
	...
	catalog := f4settings.Catalog{ID: "test", Categories: settingsCategories, Fields: []f4settings.Field{f4settings.Scalar("theme", "appearance", "Colors", "Theme", "Color palette", f4settings.String)}, Collections: cols}
...
func TestSettingsSchemeEnumerationAndWorktreeIdentity(t *testing.T) {
	...
	defer func() { AppConfig.EditorColorerCatalog = old }()
	dir := t.TempDir()
	AppConfig.EditorColorerCatalog = dir
