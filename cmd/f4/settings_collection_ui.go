	...
	for _, s := range c.sessions {
		for _, col := range s.catalog.Collections {
			...
			for _, action := range col.Actions {
				button(action.Label.Resolve(AppConfig.Language, Msg), func() {
...
func (c *settingsCenter) addCommands(category string) {
	for _, s := range c.sessions {
		for _, cmd := range s.catalog.Commands {
			...
			button := vtui.NewButton(0, 0, cmd.Label.Resolve(AppConfig.Language, Msg))
...
func settingsCollectionField(col f4settings.Collection) f4settings.Field {
	...
	for _, field := range col.Fields {
		f.Aliases = append(f.Aliases, field.Label.Resolve(AppConfig.Language, Msg), field.Description.Resolve(AppConfig.Language, Msg))
		for _, choice := range field.Choices {
			f.Aliases = append(f.Aliases, choice.Label.Resolve(AppConfig.Language, Msg))
		}
	}
	return f
}
