...
func TestSettingsRadiosInteractionLayoutAndPalette(t *testing.T) {
	...
	for _, language := range []string{"en", "ru"} {
		...
		if !strings.Contains(c.help.text, row.field.Choices[1].Description.Resolve(language, Msg)) {
		...
		b := radios.buttons[2]
		c.ProcessMouse(&vtinput.InputEvent{Type: vtinput.MouseEventType, MouseX: int16(b.X1), MouseY: int16(b.Y1), MouseEventFlags: vtinput.MouseMoved})
		if d.Values[row.field.ID] != "1" || !strings.Contains(c.help.text, row.field.Choices[2].Description.Resolve(language, Msg)) {
...
func TestSettingsRadioUnknownValueAndMouseSelection(t *testing.T) {
	...
	defer d.Close()
	c := newSettingsCenter([]*settingsSession{{catalog: f4settings.Catalog{ID: "test", Categories: settingsCategories, Fields: []f4settings.Field{f}}, draft: d}})
	...
	b := r.buttons[0]
	c.ProcessMouse(&vtinput.InputEvent{Type: vtinput.MouseEventType, KeyDown: true, ButtonState: vtinput.FromLeft1stButtonPressed, MouseX: int16(b.X1), MouseY: int16(b.Y1)})
	other := r.buttons[1]
	c.ProcessMouse(&vtinput.InputEvent{Type: vtinput.MouseEventType, KeyDown: true, ButtonState: vtinput.FromLeft1stButtonPressed, MouseX: int16(other.X1), MouseY: int16(other.Y1), MouseEventFlags: vtinput.MouseMoved})
	c.ProcessMouse(&vtinput.InputEvent{Type: vtinput.MouseEventType, MouseX: int16(other.X1), MouseY: int16(other.Y1)})
