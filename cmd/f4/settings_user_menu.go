...
func openUserMenuSettings(state *userMenuState, current *vtui.VMenu, index int, create, submenu bool) bool {
	if vtui.FrameManager == nil {
		return false
	}
	scope := "local"
	if state.mode == MenuModeMain {
		scope = "main"
	} else if state.mode == MenuModeFar {
		scope = "binary"
	}
	prefix := "menu." + scope + "."
	...
	var walk func([]UserMenuItem, []int, string)
	walk = func(items []UserMenuItem, path []int, parentID string) {
	...
	if center, ok := vtui.FrameManager.GetTopFrame().(*settingsCenter); ok && state.pf != nil {
		closed := center.OnResult
		center.OnResult = func(result int) {
			closed(result)
			if current != nil {
				current.Close()
			}
			items := state.rootItems
			for _, idx := range state.path {
