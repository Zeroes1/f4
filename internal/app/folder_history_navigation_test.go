...
func TestFolderHistoryNavigationAndMenuDoNotReorderHistory(t *testing.T) {
	...
	paneltest.WaitForLoad(t, pnl)
	if !panel.SameFolderHistoryPath(pnl.Vfs.GetPath(), middle) {
	...
	paneltest.WaitForLoad(t, pnl)
	if !panel.SameFolderHistoryPath(pnl.Vfs.GetPath(), newest) {
	...
	if !ok || !panel.SameFolderHistoryPath(selected.Name, middle) {
