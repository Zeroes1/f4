...
func TestTerminalViewSemanticModelPrimaryScreen(t *testing.T) {
	...
	if len(model.Rows) != 2 || model.Rows[0].Index != 1 || model.Rows[1].Index != 2 {
		t.Fatalf("unexpected primary-screen rows: %#v", model.Rows)
	}
	if len(model.Rows[1].Runs) < 2 || model.Rows[1].Runs[0].Text != "A" || model.Rows[1].Runs[1].Text != "B" {
		t.Fatalf("unexpected semantic runs: %#v", model.Rows[1].Runs)
	}
}
