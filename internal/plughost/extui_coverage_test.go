...
func TestExtUiRendererQueuesAndFlushesUpdates(t *testing.T) {
	...
	if len(messages) != 5 {
		t.Fatalf("got %d messages, want title plus palette/frame/scene/cursor", len(messages))
	}
	for i, want := range []string{"title", "palette", "frame", "scene", "cursor"} {
		if got := extUiString(messages[i], "type"); got != want {
			t.Errorf("message %d type = %q, want %q", i, got, want)
		}
	}
	if messages[1]["colors"] == nil || !extUiBool(messages[2], "full") {
